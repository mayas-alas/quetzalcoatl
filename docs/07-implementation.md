...
- The latest Windows executables are present under `target/release`; no complete `dist` candidate is declared because the configured Wide Linux builder (`gnx-node`) has no Linux Rust toolchain.
...
1. Install/pin a Linux Rust toolchain in the selected Wide Linux builder, without silently changing the builder distro.
