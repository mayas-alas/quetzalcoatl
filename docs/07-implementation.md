...
Deliver `gnx.exe`, `gnx-service.exe`, `gnx-setup.exe`, dedicated account/service, protected pipe, fixed Wide Linux import, secret channel and parity fixtures.
...
- The configured Wide Linux builder (`gnx-node`) now produces the Linux artifact and `.run`; a sealed `LAB_ONLY`/controlled-preview candidate is generated under `dist` with an external rootfs pair.
- Windows setup now performs post-bootstrap `doctor` and `status` observation before returning `SETUP_READY`; missing runtime config remains `ACTION_REQUIRED`. Production release remains blocked on production signing, clean-host installation, reboot, recovery and G0-G6 evidence.
...
1. Keep the selected Wide Linux builder pinned and rebuild from a clean checkout.
2. Run script/static gates from a clean checkout and keep their evidence.
3. Rebuild the complete candidate with a production rootfs, seal the manifest with the production key and record SHA256/SBOM evidence.
