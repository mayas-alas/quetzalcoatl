...
#[test]
fn strict_input_never_echoes_misplaced_secrets() {
...
#[test]
fn rejects_ambiguous_networks_paths_and_unpinned_images() {
    let original = config();
    for change in 0..7 {
        ...
        match change {
            ...
            5 => c.images.compute = "docker.io/dockurr/proxmox:latest".into(),
            _ => c.network.tailnet_ip = Some("192.168.1.2".parse().unwrap()),
        }
...
#[test]
fn generated_runtime_keeps_tls_and_persistent_compute() {
    let mut c = config();
    c.network.tailnet_ip = Some("100.64.0.10".parse().unwrap());
    ...
    let compute = &artifacts
        ...
        .unwrap()
        .content;
    assert!(compute.contains("--privileged --systemd=always"));
    ...
    let control = &artifacts
        ...
        .unwrap()
        .content;
    assert!(control.contains("https://proxmox.gnx"));
    assert!(control.contains("tls_server_name gnx-compute"));
    ...
    let zone = &artifacts
        .iter()
        .find(|a| a.path.ends_with("gnx.db"))
        .unwrap()
        .content;
...
#[test]
fn change_detection_is_idempotent_and_preserves_unmanaged_files() {
    ...
    assert_eq!(
        plan::changes(&c, &artifacts).unwrap().len(),
        artifacts.len()
    );
    for a in &artifacts {
...
#[test]
fn compute_can_be_planned_before_access_enrollment() {
    let c = config();
    assert!(
        plan::render(&c, "compute")
            .unwrap()
            .iter()
            .any(|a| a.capability == "compute")
    );
    assert_eq!(
        plan::render(&c, "control").unwrap_err().code,
        "ACCESS_IP_REQUIRED"
    );
}
