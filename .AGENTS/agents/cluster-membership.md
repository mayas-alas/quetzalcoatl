...
Keep the existing idempotent `pvecm add` path and surround it with persisted prepare, authorize, verify and confirm phases. Allow multiple members without using member count as a topology limit. A new node becomes a member whenever at least one valid online controller is visible and selects deterministically by stable Tailscale node ID. Confirm cluster visibility without a new GNX endpoint.
...
- schema migration, controller failover, QDevice or HA logic;
