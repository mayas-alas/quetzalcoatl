...
|---|---|---|---|
| HCI-1 | Hosted-CI build/release of the Windows EXE (`QuetzalcoatlSetup.exe`) crosses `SCOPE.md`'s exclusion "Hosted CI on another Windows host" and AGENTS.md ("`installer/build.ps1 -QaSigning` never runs on hosted CI", no hosted CI runners, private keys never in repo). A signed hosted-runner release also conflicts with the production-trust/private-key invariants. | Keep release builds local (`installer/build.ps1 -QaSigning`). An experimental **unsigned** `-AllowUnsigned` hosted-runner build is being validated on `agent/c/hosted-ci-build` purely for feedback; it is not releasable and carries no private key. Production signing/release from hosted CI requires a recorded scope amendment first. | |
