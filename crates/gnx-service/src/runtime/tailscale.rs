...
pub(super) fn candidate_hostname() -> Result<String, GateError> {
...
pub(super) fn prepare_tailscale(
    podman: &Path,
    hostname: &str,
    auth_key: &str,
) -> Result<(), GateError> {
...
pub(super) fn start_tailscale(podman: &Path) -> Result<(), GateError> {
...
pub(super) fn disable_tailscale_ssh(podman: &Path) -> Result<(), GateError> {
    machine_stdin(
        podman,
        [
            "podman",
...
pub(super) fn verify_tailscale_secret_cleanup(podman: &Path) -> Result<(), GateError> {
...
pub(super) fn wait_for_tailscale(
    podman: &Path,
    hostname: &str,
    tailnet: &str,
...
pub(super) fn stabilize_host_inventory(
    podman: &Path,
...
pub(super) fn read_tailscale_status(
    podman: &Path,
    hostname: &str,
    tailnet: &str,
...
pub(super) fn parse_tailscale_status(
    bytes: &[u8],
    hostname: &str,
    tailnet: &str,
) -> Result<TailscaleIdentity, String> {
...
pub(super) fn parse_serve_status(bytes: &[u8], host_port: &str) -> Result<(), String> {
...
pub(super) fn tailscale_diagnostics(podman: &Path) -> String {
