...
pub(super) fn ensure_machine_running(podman: &Path) -> Result<(), GateError> {
    let inspect = inspect_machine(podman)?;
    if inspect.state != "running" {
...
pub(super) fn wait_for_machine_ssh(podman: &Path) -> Result<(), GateError> {
...
pub(super) fn read_runtime_generation(podman: &Path) -> Result<Option<String>, GateError> {
    let script =
...
pub(super) fn write_runtime_generation(podman: &Path) -> Result<(), GateError> {
    let script = format!(
...
pub(super) fn read_managed_tailscale_state(
    podman: &Path,
...
pub(super) fn restore_managed_tailscale_state(
    podman: &Path,
    state: &[u8],
) -> Result<(), GateError> {
...
pub(super) fn remove_managed_machine(podman: &Path) -> Result<(), GateError> {
    let inspect = inspect_machine(podman)?;
    if inspect.state == "running" {
...
pub(super) fn installed_machine_image(image: &MachineImage) -> Result<PathBuf, GateError> {
...
pub(super) fn verify_artifact(path: &Path, image: &MachineImage) -> Result<bool, GateError> {
...
pub(super) fn inspect_machine(podman: &Path) -> Result<MachineInspect, GateError> {
...
pub(super) fn validate_fedora(podman: &Path) -> Result<(), GateError> {
...
pub(super) fn configure_machine_outer_mtu(podman: &Path) -> Result<(), GateError> {
...
pub(super) fn configure_pod_network_mtu(podman: &Path) -> Result<(), GateError> {
...
pub(super) fn validate_devices(podman: &Path) -> Result<(), GateError> {
