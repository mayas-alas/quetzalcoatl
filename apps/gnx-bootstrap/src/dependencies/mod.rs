...
use crate::recovery::{InstallJournal, log_path};
use crate::windows;
...
pub fn install(
    ...
    let mut journal = InstallJournal::load(requested_operation).map_err(journal_failure)?;
    ...
    let staged = staging::stage(&request)
...
fn post_validate(dependency: Dependency) -> Result<(), InstallFailure> {
    match dependency {
        Dependency::Wsl => {
            ...
            match crate::host::checks::wsl() {
                crate::host::checks::Verdict::Pass(_) => Ok(()),
                crate::host::checks::Verdict::Fail(message)
                | crate::host::checks::Verdict::Error(message)
                | crate::host::checks::Verdict::Reboot(message) => Err(InstallFailure {
...
fn journal_failure(error: crate::recovery::JournalError) -> InstallFailure {
