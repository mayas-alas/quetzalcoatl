...
use gnx_contracts::{InstallerConfiguration, NodeRole, StatusResponse};
use zeroize::Zeroize;
...
use crate::infrastructure::remote::RuntimeOperation;
use crate::infrastructure::runtime_assets::*;
use crate::infrastructure::tailscale::*;

#[test]
fn parses_locked_wsl_machine_image() {
    let manifest = include_bytes!("../../../../runtime/payload.lock.json");
    let image = parse_machine_image(manifest).expect("machine image pin");
    assert_eq!(image.artifact, "podman-machine.x86_64.wsl.tar.zst");
    assert!(image.size > 1024 * 1024);
    assert!(valid_sha256(&image.sha256));
}
...
fn payload_manifest_matches_all_installed_files() {
    ...
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../runtime");
...
fn payload_contract_mismatch_reports_both_sides() {
    ...
    assert!(
        error
            .message
            .contains(&format!("service_version={}", env!("CARGO_PKG_VERSION")))
    );
...
fn member_retry_requires_the_pinned_controller_to_remain_direct_and_available() {
    let controller = crate::infrastructure::state::ControllerIdentity {
        id: "controller".into(),
        hostname: "gnx-controller-a".into(),
        ip: "100.100.10.21".parse().expect("IP"),
    };
    let state = crate::infrastructure::state::PersistedState::member(
        "member".into(),
...
fn member_stage_status_keeps_local_components_ready() {
    ...
    assert_eq!(value.role, Some(NodeRole::Member));
    assert_eq!(
...
fn member_join_input_is_exactly_five_newline_delimited_values_and_zeroizable() {
    let member = crate::infrastructure::state::PersistedState::member(
        "member".into(),
        "100.100.10.22".parse().expect("IP"),
        "gnx-member-member".into(),
        crate::infrastructure::state::ControllerIdentity {
...
fn member_join_checkpoint_resumes_without_rediscovery_or_controller_change() {
    ...
    let mut member = crate::infrastructure::state::PersistedState::member(
        "member".into(),
        "100.100.10.22".parse().expect("IP"),
        "gnx-member-member".into(),
        controller.clone(),
    ...
    assert_eq!(
        member.cluster_join,
        crate::infrastructure::state::ClusterJoinState::Joining
    );
...
fn reconciles_reauthenticated_node_id_only_with_logical_identity_continuity() {
    let state = crate::infrastructure::state::PersistedState::controller(
        "node-id-before".into(),
