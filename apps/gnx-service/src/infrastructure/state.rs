...
const SCHEMA_VERSION: u8 = gnx_contracts::PERSISTED_STATE_SCHEMA_VERSION;
...
pub fn store(state: &PersistedState) -> Result<(), StateError> {
    ...
    let path = state_path()?;
    crate::infrastructure::secrets::atomic_write(&path, &bytes)
...
fn state_path() -> Result<std::path::PathBuf, StateError> {
    crate::infrastructure::secrets::product_root()
...
impl StateError {
    ...
    fn from_storage(error: crate::infrastructure::secrets::ConfigurationError) -> Self {
