...
use crate::infrastructure::models::{
    LockedPayloadFile, MachineImage, PayloadFile, RuntimeManifest,
};
use crate::infrastructure::remote::machine_stdin;
use crate::infrastructure::runtime_assets::{PAYLOAD_FILES, PAYLOAD_HEREDOC};

pub(crate) fn apply_runtime_payload(podman: &Path) -> Result<(), GateError> {
...
pub(crate) fn payload_install_script(file: &PayloadFile) -> Result<Vec<u8>, GateError> {
...
pub(crate) fn load_machine_image() -> Result<MachineImage, GateError> {
    let manifest = runtime_root()?.join("payload.lock.json");
...
pub(crate) fn runtime_root() -> Result<PathBuf, GateError> {
...
pub(crate) fn load_payload_files() -> Result<Vec<PayloadFile>, GateError> {
    let root = runtime_root()?;
    let manifest_path = root.join("payload.lock.json");
...
pub(crate) fn parse_payload_manifest(bytes: &[u8]) -> Result<Vec<LockedPayloadFile>, GateError> {
    ...
    if manifest.payload_version != 6 || manifest.files.len() != PAYLOAD_FILES.len() {
        return Err(GateError::new(
            "RUNTIME_PAYLOAD_INVALID",
            Component::Proxmox,
            format!(
                "runtime/service payload contract mismatch: service_version={} expected_payload_version=6 expected_files={} manifest_payload_version={} manifest_files={}",
...
pub(crate) fn valid_file_sha256(value: &str) -> bool {
    value.len() == 64
        && value
            .bytes()
...
pub(crate) fn parse_machine_image(bytes: &[u8]) -> Result<MachineImage, GateError> {
    let manifest: RuntimeManifest = serde_json::from_slice(bytes).map_err(|error| {
        GateError::new(
            "RUNTIME_PAYLOAD_INVALID",
            Component::PodmanMachine,
    ...
    let version = image.version.as_deref().unwrap_or_default();
    let artifact = image.artifact.as_deref().unwrap_or_default();
    let artifact_url = image.artifact_url.as_deref().unwrap_or_default();
    let artifact_size = image.artifact_size.unwrap_or_default();
    if image.kind.as_deref() != Some("oci_machine_image")
        || version.is_empty()
        || image.source_ref.as_deref() != Some(format!("v{version}").as_str())
        || !image.source_commit.as_deref().is_some_and(valid_git_commit)
        ...
        || artifact.is_empty()
        || artifact.contains(['/', '\\'])
        || !artifact.ends_with(".wsl.tar.zst")
        || !artifact_url.starts_with(
            "https://github.com/podman-container-tools/podman-machine-os/releases/download/",
        )
        || !artifact_url.ends_with(artifact)
        || !(1024 * 1024..=2 * 1024 * 1024 * 1024).contains(&artifact_size)
...
fn valid_sha256_digest(value: &str) -> bool {
    value.strip_prefix("sha256:").is_some_and(valid_file_sha256)
}
