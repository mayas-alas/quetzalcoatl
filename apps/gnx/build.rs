...
fn main() {
    ...
    let repo_root = manifest_dir
        ...
        .expect("gnx must remain under apps/")
        .to_path_buf();
    let icon = repo_root
        .join("installer")
        .join("assets")
    ...
    let cli_resource = compile_resource(
        &compiler,
        &out_dir,
        &icon,
        "gnx",
    ...
    let tray_resource = compile_resource(
        &compiler,
        &out_dir,
        &icon,
        "gnx-tray",
    ...
    println!("cargo:rustc-link-arg-bin=gnx={}", cli_resource.display());
    println!(
        "cargo:rustc-link-arg-bin=gnx-tray={}",
        tray_resource.display()
    );
}
...
fn compile_resource(
    ...
    let source = out_dir.join(format!("{stem}.rc"));
    let output = out_dir.join(format!("{stem}.res"));
...
BEGIN
    ...
    BEGIN
        BLOCK "040904B0"
        BEGIN
            VALUE "CompanyName", "Hector AB\0"
            VALUE "FileDescription", "{description}\0"
            VALUE "FileVersion", "{version}\0"
            VALUE "InternalName", "{stem}\0"
            VALUE "LegalCopyright", "Copyright (c) 2008-2020 Hector AB and other contributors\0"
