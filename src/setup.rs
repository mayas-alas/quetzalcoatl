fn main() {
    ...
    if args.is_empty() || args == ["--help"] {
        println!("runtime-setup preflight\nruntime-setup install <rootfs.tar> <sha256> <client-SID>\n\nInstall requires an elevated console and an Ubuntu 24.04 rootfs verified against a trusted SHA256.\nThe companion runtime.exe must be beside this EXE. This is an experimental installer.\nNo changes are made by preflight. Reboot-required exit code: 3010."); return;
    }
    #[cfg(windows)] let result = runtime_control::windows::setup(&args);
