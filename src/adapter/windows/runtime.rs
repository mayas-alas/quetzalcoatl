...
fn command(
    ...
    let output =
        super::super::process::run(WSL, args, input, timeout, 1024 * 1024).map_err(|e| {
            if e == "PROCESS_TIMEOUT" {
                "WSL_COMMAND_TIMEOUT"
            } else {
                failure
            }
    ...
    match output.code {
        0 => Ok(output.stdout.to_vec()),
        3010 => Err("REBOOT_REQUIRED".into()),
        _ => Err(failure.into()),
    }
}
fn has_distro(bytes: &[u8], wanted: &str) -> bool {
    ...
    text.lines()
        ...
        .any(|line| line == wanted || line.trim_start_matches('*').trim() == wanted)
}
...
pub fn bootstrap() -> Result<(), String> {
    ...
    let rootfs = root.join("rootfs.tar");
    if !bundle.exists() && !rootfs.exists() {
        return Ok(());
    }
    if !bundle.is_file() {
        return Err("BUNDLE_MISSING".into());
    }
    if std::fs::metadata(&bundle)
        .map_err(|_| "BUNDLE_READ_FAILED")?
        .len()
        > MAX_BOOTSTRAP_BUNDLE_BYTES
    {
        return Err("BUNDLE_TOO_LARGE".into());
    }
    command(
        &["--status"],
        None,
    ...
    if !has_distro(&listed, DISTRO) {
        if !rootfs.is_file() {
            return Err("WSL_RUNTIME_UNAVAILABLE".into());
        }
        command(
            &[
                "--import",
    ...
    command(
        &[
            "-d", DISTRO, "-u", "root", "--exec", "/bin/tar", "-xf", "-", "-C", "/",
        ],
        Some(&tar),
    ...
    if rootfs.exists() {
        std::fs::remove_file(&rootfs).map_err(|_| "BOOTSTRAP_CLEANUP_FAILED")?;
    }
    Ok(())
}
#[cfg(test)]
